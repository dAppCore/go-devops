...
func exampleCoolifyClient(result map[string]any) *Client {
	return &Client{call: func(_ core.Context, operation string, params map[string]any) (map[string]any, core.Result) {
		if result != nil {
			return result, core.Ok(nil)
		}
		return map[string]any{"operation": operation, "params": params}, core.Ok(nil)
	}}
}
...
func ExampleClient_Call() {
	...
	core.Println(err.OK, result["operation"])
...
func ExampleClient_ListServers() {
	...
	core.Println(err.OK, items[0]["uuid"])
	// Output: true list-servers
}

func ExampleClient_GetServer() {
	item, err := exampleCoolifyClient(nil).GetServer(core.Background(), "srv-1")
	core.Println(err.OK, item["operation"], item["params"].(map[string]any)["uuid"])
...
func ExampleClient_ValidateServer() {
	...
	core.Println(err.OK, item["operation"])
...
func ExampleClient_CreateProject() {
	...
	core.Println(err.OK, item["operation"], item["params"].(map[string]any)["name"])
