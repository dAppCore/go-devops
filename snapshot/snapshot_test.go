...
func TestSnapshot_Generate_Good(t *T) {
	data, r := Generate(testManifest(), "abc123", "v1.2.3")
	AssertTrue(t, r.OK)
...
func TestSnapshot_Generate_Bad(t *T) {
	data, r := Generate(nil, "abc123", "v1.2.3")
	AssertFalse(t, r.OK)
...
func TestSnapshot_Generate_Ugly(t *T) {
	data, r := Generate(&manifest.Manifest{}, "", "")
	AssertTrue(t, r.OK)
...
func TestSnapshot_GenerateAt_Good(t *T) {
	...
	data, r := GenerateAt(testManifest(), "deadbeef", "v1.2.3", built)
	AssertTrue(t, r.OK)
...
func TestSnapshot_GenerateAt_Bad(t *T) {
	data, r := GenerateAt(nil, "deadbeef", "v1.2.3", UnixTime(0))
	AssertFalse(t, r.OK)
...
func TestSnapshot_GenerateAt_Ugly(t *T) {
	...
	data, r := GenerateAt(m, "", "", UnixTime(-1).UTC())
